---
name: build-relationship-state-machine
description: build relationship state transitions. Use this skill when romance-stage work needs this operation. Applies only within the romance Genre Pack.
category: romance
---

# /build-relationship-state-machine

## Purpose

Build relationship state transitions for `romance` projects.

## Inputs

- Active project id and `PROJECT.md`.
- `genre-context/active-pack.yaml`.
- `romance` Pack files under `genre-packs/romance/`.
...
1. Confirm the active project uses `romance` as primary or secondary genre.
...
- Output follows `romance` memory and template conventions.
