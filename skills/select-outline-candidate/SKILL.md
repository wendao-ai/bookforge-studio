---
name: select-outline-candidate
description: 选择大纲候选——触发场景：大纲 阶段需要执行该操作时使用。
category: outline
---

# /select-outline-candidate

## Purpose

Compare outline candidates and record the selected structure.

## Inputs

- Active project id from `BOOKFORGE_PROJECT` or project context.
- `projects/<project-id>/PROJECT.md`.
- Current stage artifacts relevant to `outline`.
- Active Genre Pack metadata and rules when genre-specific behavior applies.
- Author instructions from the current session.

## Outputs

- Primary output: `outline/` artifacts and registry updates.
...
4. Apply the workflow for `outline` without changing unrelated project state.
