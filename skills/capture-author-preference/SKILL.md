---
name: capture-author-preference
description: 捕获作者偏好——触发场景：sedimentation 阶段需要执行该操作时使用。
category: sedimentation
---

# /capture-author-preference

## Purpose

Record stable author preferences separately from project-specific choices.

## Inputs

- Active project id from `BOOKFORGE_PROJECT` or project context.
- `projects/<project-id>/PROJECT.md`.
- Current stage artifacts relevant to `sedimentation`.
- Active Genre Pack metadata and rules when genre-specific behavior applies.
- Author instructions from the current session.

## Outputs

- Primary output: `capability-library/` reusable assets.
...
4. Apply the workflow for `sedimentation` without changing unrelated project state.
